...
import pyaudio
import struct
from camera_io import BucketSentenceIter, default_build_vocab
...
if __name__ == '__main__':
    ...
    while True:
        img_data = default_build_vocab( )

        init_c = [('l%d_init_c'%l, (batch_size, num_hidden)) for l in range(num_lstm_layer)]
        init_h = [('l%d_init_h'%l, (batch_size, num_hidden)) for l in range(num_lstm_layer)]
        init_states = init_c + init_h
        data_val = BucketSentenceIter(img_data, buckets, batch_size, init_states)
        model4 = mx.model.FeedForward.load('model1/lip', num_epoch, ctx = mx.context.gpu(0))
        begin = time.time()
